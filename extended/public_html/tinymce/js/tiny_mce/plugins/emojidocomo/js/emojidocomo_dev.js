...
var Emojidocomo_pallet = {
	...
	init : function() {
		...
		table_html = '<table class="emojidocomo" border="0" cellspacing="0" cellpadding="0">';
		for(i = 0; i < hex_range.length; i++){
			for(j = hex_range[i][0]; j <= hex_range[i][1]; j++,n++){
				filename = j.toString(16) + '.gif';
				if(n % 16 == 1)
					table_html += '<tr>';
				table_html += '<td><p><img src="img/'+filename+'" border="0" onclick="EmojidocomoDialog.insert(\'' + filename + '\');" onmouseover="this.style.border=\'solid 1px #33f\';" onmouseout="this.style.border=\'\';" /></p></td>';
				if(n % 16 == 0)
					table_html += '</tr>';
			}
		}
