...
var Emojiau_pallet = {
	...
	init : function() {
		...
		for(i = 0; i < t.hex_range.length; i++){
			n = 1;
			t.pallet_html[i] = '<table class="emojiau" border="0" cellspacing="0" cellpadding="0" style="background-image:url(img/pallet' + i + '.png);">';
			for(k = 0; k < t.hex_range[i].length; k++){
				if(t.hex_range[i][k].length == 1){
					...
					if(n % 16 == 1)
						t.pallet_html[i] += '<tr>';
					t.pallet_html[i] += '<td><p><img src="img/16px.gif" border="0" onclick="EmojiauDialog.insert(\''+filename+'.gif\');" onmouseover="this.style.border=\'solid 1px #33f\';" onmouseout="this.style.border=\'\';" /></p></td>';
					if(n % 16 == 0)
						t.pallet_html[i] += '</tr>';
					n++;
				}else{
